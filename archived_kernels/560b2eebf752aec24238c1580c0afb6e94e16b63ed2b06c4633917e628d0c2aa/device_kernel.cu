...
extern "C" __global__ void prune_block_value_mask_kernel_kernel(const half_t* __restrict__ V, half_t* __restrict__ prune_loss, int num_blocks);
extern "C" __global__ void __launch_bounds__(256, 1) prune_block_value_mask_kernel_kernel(const half_t* __restrict__ V, half_t* __restrict__ prune_loss, int num_blocks) {
  ...
  __syncthreads();
  for (int i = 0; i < 32; ++i) {
    dense_local[i] = V[(((((((((int64_t)((int)blockIdx.x)) * ((int64_t)num_blocks)) * (int64_t)65536) + ((((int64_t)((int)blockIdx.y)) * ((int64_t)num_blocks)) * (int64_t)8192)) + (((int64_t)((int)blockIdx.z)) * (int64_t)8192)) + (((int64_t)((int)threadIdx.x)) * (int64_t)4096)) + (((int64_t)i) * (int64_t)128)) + ((int64_t)((int)threadIdx.y)))];
  }

...
extern "C" __global__ void block_compress_sparse_key_kernel_kernel(const half_t* __restrict__ K, const signed char* __restrict__ block_mask, short* __restrict__ idx_map, short* __restrict__ sparse_meta, half_t* __restrict__ sparse_nonzero, int blocks_per_head, int s, int sparse_blocks_per_head);
extern "C" __global__ void __launch_bounds__(256, 1) block_compress_sparse_key_kernel_kernel(const half_t* __restrict__ K, const signed char* __restrict__ block_mask, short* __restrict__ idx_map, short* __restrict__ sparse_meta, half_t* __restrict__ sparse_nonzero, int blocks_per_head, int s, int sparse_blocks_per_head) {
    ...
    if (!mask_val) {
      ...
      *(short2*)(meta_local + 0) = make_short2((short)0, (short)0);
      for (int i_1 = 0; i_1 < 4; ++i_1) {
        uint4 condval;
        if ((((k * 64) + ((int)threadIdx.x)) < s)) {
          condval = *(uint4*)(K + ((((((((int64_t)k) * (int64_t)8192) + ((((int64_t)((int)blockIdx.y)) * ((int64_t)s)) * (int64_t)1024)) + ((((int64_t)((int)blockIdx.x)) * ((int64_t)s)) * (int64_t)128)) + (((int64_t)((int)threadIdx.x)) * (int64_t)128)) + (((int64_t)((int)threadIdx.y)) * (int64_t)32)) + (((int64_t)i_1) * (int64_t)8)));
        } else {
      ...
      for (int i_4 = 0; i_4 < 2; ++i_4) {
        if ((short)0 <= sparse_idx) {
          if (((int)sparse_idx) < sparse_blocks_per_head) {
            *(uint4*)(sparse_nonzero + (((((((((int64_t)((int)blockIdx.y)) * ((int64_t)sparse_blocks_per_head)) * (int64_t)32768) + ((((int64_t)((int)blockIdx.x)) * ((int64_t)sparse_blocks_per_head)) * (int64_t)4096)) + (((int64_t)sparse_idx) * (int64_t)4096)) + (((int64_t)((int)threadIdx.x)) * (int64_t)64)) + (((int64_t)((int)threadIdx.y)) * (int64_t)16)) + (((int64_t)i_4) * (int64_t)8))) = *(uint4*)(sparse_local + (i_4 * 8));
          }
        }
      }
      if ((short)0 <= sparse_idx) {
        if (((int)sparse_idx) < sparse_blocks_per_head) {
          *(short2*)(sparse_meta + ((((((((int64_t)((int)blockIdx.y)) * ((int64_t)sparse_blocks_per_head)) * (int64_t)4096) + ((((int64_t)((int)blockIdx.x)) * ((int64_t)sparse_blocks_per_head)) * (int64_t)512)) + (((int64_t)sparse_idx) * (int64_t)512)) + (((int64_t)((int)threadIdx.x)) * (int64_t)8)) + (((int64_t)((int)threadIdx.y)) * (int64_t)2))) = *(short2*)(meta_local + 0);
        }
      }
    }
  }

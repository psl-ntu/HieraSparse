...
BEST_CONFIGS = {
    "NVIDIA L40S": {
        # Tile-lang: 231.37 ms 304.14 TFlops
        # CUDNN: 389.09 ms 180.86 TFlops 1.68x
        # FLASH_ATTENTION: 393.21 ms 178.96 TFlops 1.70x
        (8, 32, 8, True): {"block_M": 96, "block_N": 128, "threads": 128, "use_movmatrix": True},
        # Tile-lang: 28.34 ms 310.42 TFlops
        # CUDNN: 47.78 ms 184.11 TFlops 1.69x
...
def main(
    ...
    if not tune:
        kernel = flashattn_sp_kv(
            batch, heads, groups, dim, is_causal, **BEST_CONFIGS[device_name][(batch, heads, groups, is_causal)]
        )
    else:
        kernel = flashattn_sp_kv(batch, heads, groups, dim, is_causal)
    ...
    O_tl, lse_tl = kernel(Q, K_sp, K_E, V_sp, V_E)
    ...
    latency_tl = triton.testing.do_bench(
        lambda: kernel(Q, K_sp, K_E, V_sp, V_E),
        warmup=args.warmup,
        rep=args.rep,
    )

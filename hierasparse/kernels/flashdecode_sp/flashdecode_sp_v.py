...
BEST_CONFIGS = {
    "NVIDIA L40S": {
        # TileLang: 1.2042981669329298 ms, 3.5663653851921846 tflops
        # CUDNN: 1.57 ms 2.73 TFlops 1.31x
        # FLASH_ATTENTION: 1.54 ms 2.78 TFlops 1.28x
        (8, 32, 8): {
            "block_N": 128,
            "block_H": 8,
            "num_split": 1,
        ...
        # TileLang: 0.20182454624991533 ms, 2.6600872984755948 tflops
        # CUDNN: 0.26 ms 2.03 TFlops 1.31x
        # FLASH_ATTENTION: 0.26 ms 2.04 TFlops 1.30x
        (1, 32, 8): {
            "block_N": 32,
            "block_H": 8,
            "num_split": 32,
...
def flashdecode_sp_v(batch, heads, groups, dim, block_N, block_H, num_split, num_stages, threads, use_movmatrix):
    ...
    # key (dense)
    shape_k = [batch, groups, seq_kv, dim]
    shape_k_shared = [block_N, dim]
    ...
    def flash_attn(
        ...
        with T.Kernel(batch, heads // valid_block_H, threads=threads) as (bx, by):
            ...
            for k in T.Pipelined(loop_range, num_stages=num_stages):
                ...
                T.gemm(K_shared, Q_shared, acc_s_T, transpose_B=True, policy=mma_policy)
    ...
    def flash_attn_split(
        ...
        with T.Kernel(batch, heads // valid_block_H, num_split, threads=threads) as (bx, by, bz):
            ...
            for k in T.Pipelined(tiles_this_split, num_stages=num_stages):
                ...
                T.gemm(K_shared, Q_shared, acc_s_T, transpose_B=True, policy=T.GemmWarpPolicy.FullCol)
    ...
    def flashattn_gqa_decode_split(
        ...
        flash_attn_split(Q, K, V, V_E, glse, Output_partial)
    ...
    def flashattn_gqa_decode_no_split(
        ...
        flash_attn(Q, K, V, V_E, Output, lse_combined)
...
def main(args):
    ...
    if not tune:
        config = BEST_CONFIGS[dev_name][(batch, heads, groups)]
        kernel = flashdecode_sp_v(batch, heads, groups, dim, **config)
    else:
        kernel = flashdecode_sp_v(batch, heads, groups, dim)
    ...
    O_tl, lse_tl = kernel(Q, K, V_sp, V_E, glse, Output_partial)

    tl_latency = triton.testing.do_bench(
        lambda: kernel(Q, K, V_sp, V_E, glse, Output_partial),
        warmup=args.warmup,
        rep=args.rep,
    )
